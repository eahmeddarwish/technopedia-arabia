...
const projectsData = [{
        ...
        desc: {
            ar: "سماعة طبية رقمية مبنية على Raspberry Pi، بتستخدم شبكة عصبية مضغوطة لفحص صوت القلب وتصنيفه فورياً (طبيعي / طنين / صوت إضافي / تشويش)، مع تقرير PDF بأسلوب طبي.",
        ...
        details: {
            ar: "مشروع تخرج جماعي (KCST — Group 07) يجمع بين معالجة الإشارات الصوتية والذكاء الاصطناعي والهاردوير المدمج. الصوت يُلتقط عبر ميكروفون BOYA BY-M1 Pro متصل بسماعة طبية تقليدية، يمر بخط معالجة (تصفية Bandpass + Notch، بوابة ضوضاء، اختيار أفضل نافذة 3 ثوانٍ)، ثم يتحول لصورة Mel Spectrogram تُغذّى لشبكة CNN مضغوطة بصيغة TensorFlow Lite (INT8) تعمل بالكامل على الجهاز بدون إنترنت. النتيجة تظهر على شاشة لمس PySide6 مع شرح سريري لسبب القرار، وتُصدَّر كتقرير PDF. المشروع مكتمل وشغال فعلياً، ونُشر هنا كمصدر مفتوح بعد إعادة هيكلة الكود بالكامل لمستوى احترافي (اختبارات، توثيق، إعدادات قابلة للتخصيص).",
            en: "A KCST Group 07 capstone project combining audio signal processing, on-device AI, and embedded hardware. Audio is captured via a BOYA BY-M1 Pro microphone attached to a conventional stethoscope, run through a DSP pipeline (bandpass + notch filtering, noise gating, best 3-second window selection), converted to a Mel-spectrogram, and classified by a quantized TensorFlow Lite CNN running entirely on-device with no internet connection. The result is shown on a PySide6 touchscreen with a clinical explanation of the decision, and exported as a PDF report. The project is complete and working end to end, and is published here as an open-source reference after a full professional refactor (tests, documentation, configurable settings).",
        },
        article: {
            ar: {
                lead: "سماعة رقمية بذكاء اصطناعي بتفحص صوت القلب على الجهاز نفسه — من غير إنترنت — وبتشرح سبب قرارها على الشاشة وفي تقرير PDF.",
                sections: [{
                        h: "الفكرة",
                        p: "الهدف إن أي شخص (ممرض، طبيب، أو حتى فرد في منطقة محدودة الموارد الطبية) يقدر يستخدم سماعة رخيصة نسبياً مبنية على Raspberry Pi عشان يفحص صوت القلب ويحصل على تصنيف أولي فوري: طبيعي، طنين (murmur)، صوت قلب إضافي (S3/S4)، أو تشويش في التسجيل — قبل ما يقرر يحوّل الحالة لطبيب قلب."
                    },
                    {
                        h: "خط معالجة الإشارة",
                        flow: [
                    ...
                    },
                    {
                        h: "القرارات التقنية",
                        steps: [{
                                t: "قاعدة قرار \"الطنين أولاً\"",
                                d: "بدل الاعتماد على argmax مباشرة، النموذج بيتحقق الأول من احتمالية الـ murmur مقارنة بحد أدنى محافظ. اختيار متعمد بعد تجربة عدة قيم threshold، وصل لـ 100% نسبة اكتشاف كل حالات الطنين الحقيقية (مقابل 75% دقة إجمالية) — لأن تفويت حالة طنين حقيقية أخطر بكتير من إنذار كاذب."
                            },
                            {
                                t: "ضغط النموذج لصيغة INT8",
                                d: "النموذج اتحول من TensorFlow العادي لصيغة TensorFlow Lite مع ضغط INT8 عشان يشتغل بسرعة كافية على معالج Raspberry Pi المحدود، بعد تجربة أكتر من 9 نسخ تدريب مختلفة."
                            },
                            {
                                t: "إعادة هيكلة كاملة للكود",
                                d: "الكود الأصلي كان ملف GUI واحد ضخم (أكتر من 1700 سطر) بمسارات ثابتة على جهاز Raspberry Pi معيّن. اتقسّم لحزمة Python منظمة (معالجة إشارة، استدلال، تسجيل صوت، تقرير PDF، واجهة) قابلة للاختبار من غير أي هاردوير، مع إعدادات عبر متغيرات بيئة بدل المسارات الثابتة."
                            }
                        ]
                    },
                    {
                        h: "اكتشاف وتصحيح خطأ في الداتاسيت المستخدم للتدريب",
                        p: "أثناء تجهيز المشروع للنشر، لاحظنا إن ملفات التسميات المرفقة مع داتاسيت PASCAL \"Heartbeat Sounds\" الشهير على Kaggle فيها أخطاء منهجية في مطابقة أسماء الملفات (بادئات وهمية وتضارب في صياغة الأسماء)، تخلي جزء كبير من الصفوف غير قابل للربط الصحيح مع الملف الصوتي الحقيقي. صلّحنا الخطأ بالكامل (176 من 176، و656 من 656 صف) وتأكدنا برمجياً من دقة كل تصحيح، ونشرنا جدول التصحيح فقط (أسماء وتصنيفات، من غير صوت) في المستودع — بدون إعادة نشر الداتاسيت نفسه المحمي بحقوق الملكية."
                    }
                ],
                results: [{
                        k: "نسبة اكتشاف الطنين",
                        v: "100%"
                    },
                    {
                        k: "الدقة الإجمالية",
                ...
                note: "المشروع للأغراض التعليمية والبحثية فقط، وليس جهازاً طبياً معتمداً — أي نتيجة لازم تتأكد من طبيب مختص. الكود منشور بالكامل مفتوح المصدر ويستقبل التحسين والتطوير."
            },
            en: {
